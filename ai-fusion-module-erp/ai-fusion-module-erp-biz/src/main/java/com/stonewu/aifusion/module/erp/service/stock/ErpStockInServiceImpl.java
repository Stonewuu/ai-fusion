...
import com.stonewu.aifusion.module.erp.controller.admin.stock.vo.in.ErpStockInPageReqVO;
import com.stonewu.aifusion.module.erp.controller.admin.stock.vo.in.ErpStockInSaveReqVO;
...
import com.stonewu.aifusion.module.erp.dal.mysql.stock.ErpStockInItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.stock.ErpStockInMapper;
...
public class ErpStockInServiceImpl implements ErpStockInService {
    ...
    public Long createStockIn(ErpStockInSaveReqVO createReqVO) {
        ...
        if (stockInMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_IN_NO_EXISTS);
        }
    ...
    public void updateStockIn(ErpStockInSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(stockIn.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_IN_UPDATE_FAIL_APPROVE, stockIn.getNo());
        }
        // 1.2 校验供应商
    ...
    public void updateStockInStatus(Long id, Integer status) {
        ...
        if (stockIn.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.STOCK_IN_APPROVE_FAIL : ErrorCodeConstants.STOCK_IN_PROCESS_FAIL);
        }
    ...
    public void deleteStockIn(List<Long> ids) {
        ...
        stockIns.forEach(stockIn -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(stockIn.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_IN_DELETE_FAIL_APPROVE, stockIn.getNo());
            }
        });
    ...
    private ErpStockInDO validateStockInExists(Long id) {
        ...
        if (stockIn == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_IN_NOT_EXISTS);
        }
        return stockIn;
    }
