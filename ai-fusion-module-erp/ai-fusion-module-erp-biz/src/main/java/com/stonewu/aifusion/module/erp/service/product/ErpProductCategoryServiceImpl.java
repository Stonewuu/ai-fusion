...
import com.stonewu.aifusion.module.erp.controller.admin.product.vo.category.ErpProductCategoryListReqVO;
import com.stonewu.aifusion.module.erp.controller.admin.product.vo.category.ErpProductCategorySaveReqVO;
import com.stonewu.aifusion.module.erp.dal.dataobject.product.ErpProductCategoryDO;
import com.stonewu.aifusion.module.erp.dal.mysql.product.ErpProductCategoryMapper;
...
public class ErpProductCategoryServiceImpl implements ErpProductCategoryService {
    ...
    public void deleteProductCategory(Long id) {
        ...
        if (erpProductCategoryMapper.selectCountByParentId(id) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PRODUCT_CATEGORY_EXITS_CHILDREN);
        }
        // 1.3 校验是否有产品
        if (productService.getProductCountByCategoryId(id) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PRODUCT_CATEGORY_EXITS_PRODUCT);
        }
        // 2. 删除
    ...
    private void validateParentProductCategory(Long id, Long parentId) {
        ...
        if (Objects.equals(id, parentId)) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PRODUCT_CATEGORY_PARENT_ERROR);
        }
        // 2. 父产品分类不存在
        ErpProductCategoryDO parentCategory = erpProductCategoryMapper.selectById(parentId);
        if (parentCategory == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PRODUCT_CATEGORY_PARENT_NOT_EXITS);
        }
        ...
        for (int i = 0; i < Short.MAX_VALUE; i++) {
            ...
            if (Objects.equals(id, parentId)) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.PRODUCT_CATEGORY_PARENT_IS_CHILD);
            }
            // 3.2 继续递归下一级父产品分类
            if (parentId == null || ErpProductCategoryDO.PARENT_ID_ROOT.equals(parentId)) {
                break;
    ...
    private void validateProductCategoryNameUnique(Long id, Long parentId, String name) {
        ...
        if (id == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PRODUCT_CATEGORY_NAME_DUPLICATE);
        }
