...
import com.stonewu.aifusion.module.erp.controller.admin.stock.vo.check.ErpStockCheckSaveReqVO;
...
import com.stonewu.aifusion.module.erp.dal.mysql.stock.ErpStockCheckItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.stock.ErpStockCheckMapper;
...
public class ErpStockCheckServiceImpl implements ErpStockCheckService {
    ...
    public Long createStockCheck(ErpStockCheckSaveReqVO createReqVO) {
        ...
        if (stockCheckMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_CHECK_NO_EXISTS);
        }
    ...
    public void updateStockCheck(ErpStockCheckSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(stockCheck.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_CHECK_UPDATE_FAIL_APPROVE, stockCheck.getNo());
        }
        // 1.2 校验盘点项的有效性
    ...
    public void updateStockCheckStatus(Long id, Integer status) {
        ...
        if (stockCheck.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.STOCK_CHECK_APPROVE_FAIL : ErrorCodeConstants.STOCK_CHECK_PROCESS_FAIL);
        }
    ...
    public void deleteStockCheck(List<Long> ids) {
        ...
        stockChecks.forEach(stockCheck -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(stockCheck.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_CHECK_DELETE_FAIL_APPROVE, stockCheck.getNo());
            }
        });
    ...
    private ErpStockCheckDO validateStockCheckExists(Long id) {
        ...
        if (stockCheck == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_CHECK_NOT_EXISTS);
        }
        return stockCheck;
    }
