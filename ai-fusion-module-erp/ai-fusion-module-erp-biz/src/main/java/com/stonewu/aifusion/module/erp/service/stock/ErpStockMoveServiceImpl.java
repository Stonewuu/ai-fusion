...
import com.stonewu.aifusion.module.erp.controller.admin.stock.vo.move.ErpStockMovePageReqVO;
import com.stonewu.aifusion.module.erp.controller.admin.stock.vo.move.ErpStockMoveSaveReqVO;
...
import com.stonewu.aifusion.module.erp.dal.mysql.stock.ErpStockMoveItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.stock.ErpStockMoveMapper;
...
public class ErpStockMoveServiceImpl implements ErpStockMoveService {
    ...
    public Long createStockMove(ErpStockMoveSaveReqVO createReqVO) {
        ...
        if (stockMoveMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_MOVE_NO_EXISTS);
        }
    ...
    public void updateStockMove(ErpStockMoveSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(stockMove.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_MOVE_UPDATE_FAIL_APPROVE, stockMove.getNo());
        }
        // 1.2 校验出库项的有效性
    ...
    public void updateStockMoveStatus(Long id, Integer status) {
        ...
        if (stockMove.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.STOCK_MOVE_APPROVE_FAIL : ErrorCodeConstants.STOCK_MOVE_PROCESS_FAIL);
        }
    ...
    public void deleteStockMove(List<Long> ids) {
        ...
        stockMoves.forEach(stockMove -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(stockMove.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_MOVE_DELETE_FAIL_APPROVE, stockMove.getNo());
            }
        });
    ...
    private ErpStockMoveDO validateStockMoveExists(Long id) {
        ...
        if (stockMove == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_MOVE_NOT_EXISTS);
        }
        return stockMove;
    }
