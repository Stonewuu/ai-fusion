...
import com.stonewu.aifusion.module.erp.dal.mysql.purchase.ErpPurchaseInItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.purchase.ErpPurchaseInMapper;
...
public class ErpPurchaseInServiceImpl implements ErpPurchaseInService {
    ...
    public Long createPurchaseIn(ErpPurchaseInSaveReqVO createReqVO) {
        ...
        if (purchaseInMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_IN_NO_EXISTS);
        }
    ...
    public void updatePurchaseIn(ErpPurchaseInSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(purchaseIn.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_IN_UPDATE_FAIL_APPROVE, purchaseIn.getNo());
        }
        // 1.2 校验采购订单已审核
    ...
    public void updatePurchaseInStatus(Long id, Integer status) {
        ...
        if (purchaseIn.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.PURCHASE_IN_APPROVE_FAIL : ErrorCodeConstants.PURCHASE_IN_PROCESS_FAIL);
        }
        // 1.3 校验已付款
        if (!approve && purchaseIn.getPaymentPrice().compareTo(BigDecimal.ZERO) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_IN_PROCESS_FAIL_EXISTS_PAYMENT);
        }
    ...
    public void updatePurchaseInPaymentPrice(Long id, BigDecimal paymentPrice) {
        ...
        if (paymentPrice.compareTo(purchaseIn.getTotalPrice()) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_IN_FAIL_PAYMENT_PRICE_EXCEED, paymentPrice, purchaseIn.getTotalPrice());
        }
    ...
    public void deletePurchaseIn(List<Long> ids) {
        ...
        purchaseIns.forEach(purchaseIn -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(purchaseIn.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_IN_DELETE_FAIL_APPROVE, purchaseIn.getNo());
            }
        });
    ...
    private ErpPurchaseInDO validatePurchaseInExists(Long id) {
        ErpPurchaseInDO purchaseIn = purchaseInMapper.selectById(id);
        if (purchaseIn == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_IN_NOT_EXISTS);
        }
        return purchaseIn;
    }
    ...
    public ErpPurchaseInDO validatePurchaseIn(Long id) {
        ...
        if (ObjectUtil.notEqual(purchaseIn.getStatus(), ErpAuditStatus.APPROVE.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_IN_NOT_APPROVE);
        }
        return purchaseIn;
    }
