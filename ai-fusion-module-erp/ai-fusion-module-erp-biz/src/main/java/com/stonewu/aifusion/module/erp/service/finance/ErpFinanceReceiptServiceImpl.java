...
import com.stonewu.aifusion.module.erp.controller.admin.finance.vo.receipt.ErpFinanceReceiptSaveReqVO;
...
import com.stonewu.aifusion.module.erp.dal.mysql.finance.ErpFinanceReceiptItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.finance.ErpFinanceReceiptMapper;
...
import com.stonewu.aifusion.module.erp.service.sale.ErpSaleReturnService;
...
public class ErpFinanceReceiptServiceImpl implements ErpFinanceReceiptService {
    ...
    public Long createFinanceReceipt(ErpFinanceReceiptSaveReqVO createReqVO) {
        ...
        if (financeReceiptMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.FINANCE_RECEIPT_NO_EXISTS);
        }
    ...
    public void updateFinanceReceipt(ErpFinanceReceiptSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(receipt.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.FINANCE_RECEIPT_UPDATE_FAIL_APPROVE, receipt.getNo());
        }
        // 1.2 校验客户
    ...
    public void updateFinanceReceiptStatus(Long id, Integer status) {
        ...
        if (receipt.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.FINANCE_RECEIPT_APPROVE_FAIL : ErrorCodeConstants.FINANCE_RECEIPT_PROCESS_FAIL);
        }
    ...
    public void deleteFinanceReceipt(List<Long> ids) {
        ...
        receipts.forEach(receipt -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(receipt.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.FINANCE_RECEIPT_DELETE_FAIL_APPROVE, receipt.getNo());
            }
        });
    ...
    private ErpFinanceReceiptDO validateFinanceReceiptExists(Long id) {
        ...
        if (receipt == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.FINANCE_RECEIPT_NOT_EXISTS);
        }
        return receipt;
    }
