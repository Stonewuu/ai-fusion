...
import com.stonewu.aifusion.module.erp.controller.admin.purchase.vo.order.ErpPurchaseOrderPageReqVO;
import com.stonewu.aifusion.module.erp.controller.admin.purchase.vo.order.ErpPurchaseOrderSaveReqVO;
...
import com.stonewu.aifusion.module.erp.dal.mysql.purchase.ErpPurchaseOrderItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.purchase.ErpPurchaseOrderMapper;
...
public class ErpPurchaseOrderServiceImpl implements ErpPurchaseOrderService {
    ...
    public Long createPurchaseOrder(ErpPurchaseOrderSaveReqVO createReqVO) {
        ...
        if (purchaseOrderMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_ORDER_NO_EXISTS);
        }
    ...
    public void updatePurchaseOrder(ErpPurchaseOrderSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(purchaseOrder.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_ORDER_UPDATE_FAIL_APPROVE, purchaseOrder.getNo());
        }
        // 1.2 校验供应商
    ...
    public void updatePurchaseOrderStatus(Long id, Integer status) {
        ...
        if (purchaseOrder.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.PURCHASE_ORDER_APPROVE_FAIL : ErrorCodeConstants.PURCHASE_ORDER_PROCESS_FAIL);
        }
        // 1.3 存在采购入单，无法反审核
        if (!approve && purchaseOrder.getInCount().compareTo(BigDecimal.ZERO) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_ORDER_PROCESS_FAIL_EXISTS_IN);
        }
        // 1.4 存在采购退货单，无法反审核
        if (!approve && purchaseOrder.getReturnCount().compareTo(BigDecimal.ZERO) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_ORDER_PROCESS_FAIL_EXISTS_RETURN);
        }
    ...
    public void updatePurchaseOrderInCount(Long id, Map<Long, BigDecimal> inCountMap) {
        ...
        orderItems.forEach(item -> {
            ...
            if (inCount.compareTo(item.getCount()) > 0) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_ORDER_ITEM_IN_FAIL_PRODUCT_EXCEED,
    ...
    public void updatePurchaseOrderReturnCount(Long orderId, Map<Long, BigDecimal> returnCountMap) {
        ...
        orderItems.forEach(item -> {
            ...
            if (returnCount.compareTo(item.getInCount()) > 0) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_ORDER_ITEM_RETURN_FAIL_IN_EXCEED,
    ...
    public void deletePurchaseOrder(List<Long> ids) {
        ...
        purchaseOrders.forEach(purchaseOrder -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(purchaseOrder.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_ORDER_DELETE_FAIL_APPROVE, purchaseOrder.getNo());
            }
        });
    ...
    private ErpPurchaseOrderDO validatePurchaseOrderExists(Long id) {
        ...
        if (purchaseOrder == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_ORDER_NOT_EXISTS);
        }
    ...
    public ErpPurchaseOrderDO validatePurchaseOrder(Long id) {
        ...
        if (ObjectUtil.notEqual(purchaseOrder.getStatus(), ErpAuditStatus.APPROVE.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PURCHASE_ORDER_NOT_APPROVE);
        }
        return purchaseOrder;
    }
