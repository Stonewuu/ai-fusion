...
import com.stonewu.aifusion.module.erp.controller.admin.sale.vo.order.ErpSaleOrderPageReqVO;
import com.stonewu.aifusion.module.erp.controller.admin.sale.vo.order.ErpSaleOrderSaveReqVO;
...
import com.stonewu.aifusion.module.erp.dal.dataobject.sale.ErpSaleOrderItemDO;
import com.stonewu.aifusion.module.erp.dal.mysql.sale.ErpSaleOrderItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.sale.ErpSaleOrderMapper;
...
public class ErpSaleOrderServiceImpl implements ErpSaleOrderService {
    ...
    public Long createSaleOrder(ErpSaleOrderSaveReqVO createReqVO) {
        ...
        if (saleOrderMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_ORDER_NO_EXISTS);
        }
    ...
    public void updateSaleOrder(ErpSaleOrderSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(saleOrder.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_ORDER_UPDATE_FAIL_APPROVE, saleOrder.getNo());
        }
        // 1.2 校验客户
    ...
    public void updateSaleOrderStatus(Long id, Integer status) {
        ...
        if (saleOrder.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.SALE_ORDER_APPROVE_FAIL : ErrorCodeConstants.SALE_ORDER_PROCESS_FAIL);
        }
        // 1.3 存在销售出库单，无法反审核
        if (!approve && saleOrder.getOutCount().compareTo(BigDecimal.ZERO) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_ORDER_PROCESS_FAIL_EXISTS_OUT);
        }
        // 1.4 存在销售退货单，无法反审核
        if (!approve && saleOrder.getReturnCount().compareTo(BigDecimal.ZERO) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_ORDER_PROCESS_FAIL_EXISTS_RETURN);
        }
    ...
    public void updateSaleOrderOutCount(Long id, Map<Long, BigDecimal> outCountMap) {
        ...
        orderItems.forEach(item -> {
            ...
            if (outCount.compareTo(item.getCount()) > 0) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_ORDER_ITEM_OUT_FAIL_PRODUCT_EXCEED,
    ...
    public void updateSaleOrderReturnCount(Long orderId, Map<Long, BigDecimal> returnCountMap) {
        ...
        orderItems.forEach(item -> {
            ...
            if (returnCount.compareTo(item.getOutCount()) > 0) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_ORDER_ITEM_RETURN_FAIL_OUT_EXCEED,
    ...
    public void deleteSaleOrder(List<Long> ids) {
        ...
        saleOrders.forEach(saleOrder -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(saleOrder.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_ORDER_DELETE_FAIL_APPROVE, saleOrder.getNo());
            }
        });
    ...
    private ErpSaleOrderDO validateSaleOrderExists(Long id) {
        ...
        if (saleOrder == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_ORDER_NOT_EXISTS);
        }
        return saleOrder;
    }
    ...
    public ErpSaleOrderDO validateSaleOrder(Long id) {
        ...
        if (ObjectUtil.notEqual(saleOrder.getStatus(), ErpAuditStatus.APPROVE.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_ORDER_NOT_APPROVE);
        }
        return saleOrder;
    }
