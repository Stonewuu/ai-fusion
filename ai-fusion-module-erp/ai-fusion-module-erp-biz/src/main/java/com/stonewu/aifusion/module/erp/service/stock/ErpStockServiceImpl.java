...
import com.stonewu.aifusion.module.erp.dal.mysql.stock.ErpStockMapper;
...
public class ErpStockServiceImpl implements ErpStockService {
    ...
    public BigDecimal updateStockCountIncrement(Long productId, Long warehouseId, BigDecimal count) {
        ...
        if (!NEGATIVE_STOCK_COUNT_ENABLE && stock.getCount().add(count).compareTo(BigDecimal.ZERO) < 0) {
            throw exception(ErrorCodeConstants.STOCK_COUNT_NEGATIVE, productService.getProduct(productId).getName(),
        ...
        if (updateCount == 0) {
            ...
            throw exception(ErrorCodeConstants.STOCK_COUNT_NEGATIVE2, productService.getProduct(productId).getName(),
