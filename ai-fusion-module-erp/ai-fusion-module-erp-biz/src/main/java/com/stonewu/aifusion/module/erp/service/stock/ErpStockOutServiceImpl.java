...
import com.stonewu.aifusion.module.erp.controller.admin.stock.vo.out.ErpStockOutSaveReqVO;
...
import com.stonewu.aifusion.module.erp.dal.mysql.stock.ErpStockOutItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.stock.ErpStockOutMapper;
...
public class ErpStockOutServiceImpl implements ErpStockOutService {
    ...
    public Long createStockOut(ErpStockOutSaveReqVO createReqVO) {
        ...
        if (stockOutMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_OUT_NO_EXISTS);
        }
    ...
    public void updateStockOut(ErpStockOutSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(stockOut.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_OUT_UPDATE_FAIL_APPROVE, stockOut.getNo());
        }
        // 1.2 校验客户
    ...
    public void updateStockOutStatus(Long id, Integer status) {
        ...
        if (stockOut.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.STOCK_OUT_APPROVE_FAIL : ErrorCodeConstants.STOCK_OUT_PROCESS_FAIL);
        }
    ...
    public void deleteStockOut(List<Long> ids) {
        ...
        stockOuts.forEach(stockOut -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(stockOut.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_OUT_DELETE_FAIL_APPROVE, stockOut.getNo());
            }
        });
    ...
    private ErpStockOutDO validateStockOutExists(Long id) {
        ...
        if (stockOut == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.STOCK_OUT_NOT_EXISTS);
        }
        return stockOut;
    }
