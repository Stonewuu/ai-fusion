...
import com.stonewu.aifusion.module.erp.controller.admin.sale.vo.returns.ErpSaleReturnPageReqVO;
import com.stonewu.aifusion.module.erp.controller.admin.sale.vo.returns.ErpSaleReturnSaveReqVO;
...
import com.stonewu.aifusion.module.erp.dal.dataobject.sale.ErpSaleReturnItemDO;
import com.stonewu.aifusion.module.erp.dal.mysql.sale.ErpSaleReturnItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.sale.ErpSaleReturnMapper;
...
public class ErpSaleReturnServiceImpl implements ErpSaleReturnService {
    ...
    public Long createSaleReturn(ErpSaleReturnSaveReqVO createReqVO) {
        ...
        if (saleReturnMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_RETURN_NO_EXISTS);
        }
    ...
    public void updateSaleReturn(ErpSaleReturnSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(saleReturn.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_RETURN_UPDATE_FAIL_APPROVE, saleReturn.getNo());
        }
        // 1.2 校验销售订单已审核
    ...
    public void updateSaleReturnStatus(Long id, Integer status) {
        ...
        if (saleReturn.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.SALE_RETURN_APPROVE_FAIL : ErrorCodeConstants.SALE_RETURN_PROCESS_FAIL);
        }
        // 1.3 校验已退款
        if (!approve && saleReturn.getRefundPrice().compareTo(BigDecimal.ZERO) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_RETURN_PROCESS_FAIL_EXISTS_REFUND);
        }
    ...
    public void updateSaleReturnRefundPrice(Long id, BigDecimal refundPrice) {
        ...
        if (refundPrice.compareTo(saleReturn.getTotalPrice()) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_RETURN_FAIL_REFUND_PRICE_EXCEED, refundPrice, saleReturn.getTotalPrice());
        }
    ...
    public void deleteSaleReturn(List<Long> ids) {
        ...
        saleReturns.forEach(saleReturn -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(saleReturn.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_RETURN_DELETE_FAIL_APPROVE, saleReturn.getNo());
            }
        });
    ...
    private ErpSaleReturnDO validateSaleReturnExists(Long id) {
        ErpSaleReturnDO saleReturn = saleReturnMapper.selectById(id);
        if (saleReturn == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_RETURN_NOT_EXISTS);
        }
        return saleReturn;
    }
    ...
    public ErpSaleReturnDO validateSaleReturn(Long id) {
        ...
        if (ObjectUtil.notEqual(saleReturn.getStatus(), ErpAuditStatus.APPROVE.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_RETURN_NOT_APPROVE);
        }
        return saleReturn;
    }
