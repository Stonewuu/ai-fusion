...
import com.stonewu.aifusion.module.erp.dal.mysql.sale.ErpSaleOutItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.sale.ErpSaleOutMapper;
...
public class ErpSaleOutServiceImpl implements ErpSaleOutService {
    ...
    public Long createSaleOut(ErpSaleOutSaveReqVO createReqVO) {
        ...
        if (saleOutMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_OUT_NO_EXISTS);
        }
    ...
    public void updateSaleOut(ErpSaleOutSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(saleOut.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_OUT_UPDATE_FAIL_APPROVE, saleOut.getNo());
        }
        // 1.2 校验销售订单已审核
    ...
    public void updateSaleOutStatus(Long id, Integer status) {
        ...
        if (saleOut.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.SALE_OUT_APPROVE_FAIL : ErrorCodeConstants.SALE_OUT_PROCESS_FAIL);
        }
        // 1.3 校验已退款
        if (!approve && saleOut.getReceiptPrice().compareTo(BigDecimal.ZERO) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_OUT_PROCESS_FAIL_EXISTS_RECEIPT);
        }
    ...
    public void updateSaleInReceiptPrice(Long id, BigDecimal receiptPrice) {
        ...
        if (receiptPrice.compareTo(saleOut.getTotalPrice()) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_OUT_FAIL_RECEIPT_PRICE_EXCEED, receiptPrice,  saleOut.getTotalPrice());
        }
    ...
    public void deleteSaleOut(List<Long> ids) {
        ...
        saleOuts.forEach(saleOut -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(saleOut.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_OUT_DELETE_FAIL_APPROVE, saleOut.getNo());
            }
        });
    ...
    private ErpSaleOutDO validateSaleOutExists(Long id) {
        ErpSaleOutDO saleOut = saleOutMapper.selectById(id);
        if (saleOut == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_OUT_NOT_EXISTS);
        }
        return saleOut;
    }
    ...
    public ErpSaleOutDO validateSaleOut(Long id) {
        ...
        if (ObjectUtil.notEqual(saleOut.getStatus(), ErpAuditStatus.APPROVE.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SALE_OUT_NOT_APPROVE);
        }
        return saleOut;
    }
