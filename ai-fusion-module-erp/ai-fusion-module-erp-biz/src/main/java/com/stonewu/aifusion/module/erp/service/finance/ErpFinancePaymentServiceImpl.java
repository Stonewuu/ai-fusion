...
import com.stonewu.aifusion.module.erp.controller.admin.finance.vo.payment.ErpFinancePaymentSaveReqVO;
...
import com.stonewu.aifusion.module.erp.dal.mysql.finance.ErpFinancePaymentItemMapper;
import com.stonewu.aifusion.module.erp.dal.mysql.finance.ErpFinancePaymentMapper;
...
import com.stonewu.aifusion.module.erp.enums.common.ErpBizTypeEnum;
import com.stonewu.aifusion.module.erp.service.purchase.ErpPurchaseInService;
import com.stonewu.aifusion.module.erp.service.purchase.ErpPurchaseReturnService;
...
public class ErpFinancePaymentServiceImpl implements ErpFinancePaymentService {
    ...
    public Long createFinancePayment(ErpFinancePaymentSaveReqVO createReqVO) {
        ...
        if (financePaymentMapper.selectByNo(no) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.FINANCE_PAYMENT_NO_EXISTS);
        }
    ...
    public void updateFinancePayment(ErpFinancePaymentSaveReqVO updateReqVO) {
        ...
        if (ErpAuditStatus.APPROVE.getStatus().equals(payment.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.FINANCE_PAYMENT_UPDATE_FAIL_APPROVE, payment.getNo());
        }
        // 1.2 校验供应商
    ...
    public void updateFinancePaymentStatus(Long id, Integer status) {
        ...
        if (payment.getStatus().equals(status)) {
            throw ServiceExceptionUtil.exception(approve ? ErrorCodeConstants.FINANCE_PAYMENT_APPROVE_FAIL : ErrorCodeConstants.FINANCE_PAYMENT_PROCESS_FAIL);
        }
    ...
    public void deleteFinancePayment(List<Long> ids) {
        ...
        payments.forEach(payment -> {
            if (ErpAuditStatus.APPROVE.getStatus().equals(payment.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.FINANCE_PAYMENT_DELETE_FAIL_APPROVE, payment.getNo());
            }
        });
    ...
    private ErpFinancePaymentDO validateFinancePaymentExists(Long id) {
        ...
        if (payment == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.FINANCE_PAYMENT_NOT_EXISTS);
        }
        return payment;
    }
