package com.stonewu.aifusion.module.trade.service.price.calculator;
...
import com.stonewu.aifusion.module.trade.enums.delivery.DeliveryExpressChargeModeEnum;
...
import com.stonewu.aifusion.module.trade.service.delivery.DeliveryExpressTemplateService;
import com.stonewu.aifusion.module.trade.service.delivery.DeliveryPickUpStoreService;
import com.stonewu.aifusion.module.trade.service.delivery.bo.DeliveryExpressTemplateRespBO;
...
public class TradeDeliveryPriceCalculator implements TradePriceCalculator {
    ...
    private void calculateExpress(TradePriceCalculateReqBO param, TradePriceCalculateRespBO result) {
        ...
        // 情况二：快递模版
        // 2.1 过滤出已选中的商品 SKU
        List<TradePriceCalculateRespBO.OrderItem> selectedItem = filterList(result.getItems(), TradePriceCalculateRespBO.OrderItem::getSelected);
        Set<Long> deliveryTemplateIds = convertSet(selectedItem, TradePriceCalculateRespBO.OrderItem::getDeliveryTemplateId);
    ...
    private void calculateDeliveryPrice(List<TradePriceCalculateRespBO.OrderItem> selectedSkus,
                                        Map<Long, DeliveryExpressTemplateRespBO> expressTemplateMap,
                                        TradePriceCalculateRespBO result) {
        // 按商品运费模板来计算商品的运费：相同的运费模板可能对应多条订单商品 SKU
        Map<Long, List<TradePriceCalculateRespBO.OrderItem>> template2ItemMap = convertMultiMap(selectedSkus, TradePriceCalculateRespBO.OrderItem::getDeliveryTemplateId);
        // 依次计算快递运费
        for (Map.Entry<Long, List<TradePriceCalculateRespBO.OrderItem>> entry : template2ItemMap.entrySet()) {
            Long templateId  = entry.getKey();
            List<TradePriceCalculateRespBO.OrderItem> orderItems = entry.getValue();
     ...
     */
    private void calculateExpressFeeByChargeMode(List<TradePriceCalculateRespBO.OrderItem> orderItems, Integer chargeMode,
    ...
    private double getTotalChargeValue(List<TradePriceCalculateRespBO.OrderItem> orderItems, Integer chargeMode) {
        double total = 0;
    ...
    private double getChargeValue(TradePriceCalculateRespBO.OrderItem orderItem, Integer chargeMode) {
